...
def work(mailserver: bool = typer.Option(False)):
    ...
    try:
        ...
        typer.secho("Honcho not installed. Install with: pip install honcho", fg=typer.colors.RED)
        raise typer.Exit(1)
...
def create_user(
    ...
    """Create a new user."""
    # Lazy import for Tortoise (only needed for this command)
    try:
        from tortoise import Tortoise
        from app.db.config import TORTOISE_ORM
    except ImportError:
        typer.secho("Tortoise ORM not installed. This command requires Tortoise.", fg=typer.colors.RED)
        raise typer.Exit(1)
...
def import_stocks(
    ...
    async def _import_stocks():
        async with async_session_maker() as session:
            try:
                # Clear stocks if requested
                if clear:
                    typer.secho("Clearing existing stocks...", fg=typer.colors.YELLOW)
                    await session.execute(delete(Stock))
                    await session.commit()
                    typer.secho("Existing stocks cleared.", fg=typer.colors.GREEN)
                ...
                # Run validation
                typer.secho("Running validation...", fg=typer.colors.BLUE)
                validation = await validate_all(session, expected_count=500)
                
                if validation["valid"]:
                    typer.secho("✓ All validations passed", fg=typer.colors.GREEN)
                else:
                    typer.secho("⚠ Some validations failed:", fg=typer.colors.YELLOW)
                    for check_name, check_result in validation["checks"].items():
                        status = "✓" if check_result["valid"] else "✗"
                        typer.secho(f"  {status} {check_name}: {check_result['message']}")
            except Exception as e:
                typer.secho(f"Error importing stocks: {e}", fg=typer.colors.RED)
                raise
            finally:
                await engine.dispose()
    
    try:
        asyncio.run(_import_stocks())
    except Exception as e:
        typer.secho(f"Failed to import stocks: {e}", fg=typer.colors.RED)
        raise typer.Exit(1)
